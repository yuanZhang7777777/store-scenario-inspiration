...
Sales rows are product observations from screenshots: never sum rows across
screenshots, interpret a product-type label as a SKU identity, or invent a
reporting period.
"""
...
不要从月销量估算日均订单，不把部分商品销量冒充全店数据，不估算利润或转化率。
...
结合店铺字段、可见商品、business_context 中的商品销售证据分析。不得编造 ERP SKU、利润、市场规模、竞品表现、增长趋势或库存；具体 SKU 由后续产品库检索提供。
商品 orders、units_sold、gmv 不同，不能互换。无来源、无表头、无周期的数据不能推算日均或趋势；不同币种、周期不能混排。没有全店经营指标，也不替运营推断一个。
...
每个关键判断说清来处——是哪一类商品、哪个方向上的信号看出来的，不要逐一复述商品名和数字；截图里的商品和销量另有一处展示，正复述一遍等于没写。每条策略说明针对谁、做什么、为什么和如何观察结果，避免万能套话。
