...
工作区整理入口：[UI 基准说明](2026-09-22-ui-baseline.md)。`074aac7` 的页面移植已被回退，并保留在恢复分支；当前不要把那次移植当作已运行代码。

> 2026-09-22 已接入正式应用：首页放新建入口与历史店铺，店铺内用紧凑进度、默认展开的完整分析、章节侧栏、默认全选的场景和页尾导出完成操作。取消“处理详情”，导出配置全部可见。现行实现与验收见 [导出优先设计](2026-09-22-export-first-design.md)。旧原型已冻结，下文仅作业务约束和历史上下文；旧布局不作为本轮实现规格。
...
八步中文名存在 `app/jobs.py::STAGE_LABELS`，前端 `StorePage.tsx::PIPELINE` 映射产物与锚点；用户界面的四个紧凑节点由 `StageRail` 分组展示，不再渲染八步处理详情。步骤代码必须与后端对应，面向用户的分组文案见现行设计。
...
**长列表默认折叠。** 场景、角色、候选三层默认都是收起的，展开只渲染点开的那一个，展开时页面不许跳动。用户可以点击侧栏章节、页尾导出或导出场景摘要进行锚点滚动；后台更新不得自动滚动。
