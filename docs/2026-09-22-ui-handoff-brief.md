...
> 2026-09-22 补充：用户将首要目标明确为“最快拿到想要的导出文件”。动画仅用于等待；店铺分析生成后自动缩成顶部连续进度链，在同一界面直接展示完整分析，后台继续召回，完成后原位启用导出，不打断阅读。自绘图形与背景动效随进度区域一起收缩。新提案见 [导出优先设计](2026-09-22-export-first-design.md)，尚未接入应用。下文仅用于理解原有实现和此前方向；“锁一屏、禁止上下滑”等旧要求不约束完整分析的同页阅读。
