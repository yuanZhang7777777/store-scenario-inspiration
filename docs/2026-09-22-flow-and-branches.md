...
```
首页：新建店铺 / 历史店铺
   └─ 店铺工作页：选国家 + (截图 或 商品名) + 选填设置
   └─【开始生成】一个按钮
        └─ 建店 → 手工商品写进去 → 一个任务从①识别跑到⑧复核
                                          ↓
                                   同一工作页逐步显示分析、场景和商品 → 页尾导出 Excel
                                          ↓
                        在结果里增删改商品 / 改参数
                                          ↓
                        再按按钮，只重跑落后的那几步
...
界面将八步合为四个紧凑节点：店铺分析 / 生成场景 / 场景商品 / 匹配商品；不再显示处理详情。完整分析默认展开，侧栏定位章节，场景商品按层展开，页尾导出配置直接可见。具体布局见 [现行实现](2026-09-22-export-first-design.md)。
...
```
前端每 2 秒问一次 GET /api/jobs/{id}（上一请求未完成时不叠加）
  │
  ├─ 有新完成的步骤？
  ...
  │      scenes / products 等其余步骤完成时重新读取可用结果，让新场景和需求立即可见
  │
...
```
点开店铺
 ├─ 上次中断或有失败？ → 【继续生成】，合并未完成步骤与 outdated
 ├─ 有落后且已有分析？ → 【更新结果】，执行 outdated
 ├─ 首次尚无分析？     → 【开始生成】，执行所有缺失步骤
 ├─ 参数有未保存修改？ → 【应用设置并更新】，保存并按新 outdated 重算
 └─ 都已完成           → 阅读、调整勾选、页尾导出
```
...
| 保存参数 | `PUT /api/stores/{id}/params` | **校验后写文件**，不触发任何步骤 |
| 起任务 | `POST /api/stores/{id}/jobs` | 校验店铺非空 → 冻结参数 → 排队 |
| 历史列表 | `GET /api/stores` | 附带 job_status / needs_update；前端每 5 秒刷新 |
| 店铺详情 | `GET /api/stores/{id}` | 附带 export_blocked_reason；空串表示可导出，缺子款明细将商品匹配加入 outdated |
| 导出 | `POST /api/stores/{id}/adoption/export` | 在店铺锁内生成；缺产物或过期返回 409，占用锁时沿现有逻辑返回 400 |
