...
- 保守延展：主要使用截图中已出现的强商品簇，形成容易落地的组合。
- 相邻拓展：以强商品簇为锚点，补充用途相邻的商品角色。
- 探索机会：从相邻任务、空间或人群切入，允许出现 ERP 暂无可靠候选的角色。

每层固定输出 3 个场景，共 9 个。场景之间不能只是同义改写；需要在核心任务、使用空间、目标人群或关键商品组合上至少有一个实质差异。
...
- ERP 索引不可用：场景仍可生成，但商品角色显示“检索暂不可用”；不能把所有角色错误标为“新产品机会”。
- 没有可靠 ERP 候选：显示“新产品机会”。
...
5. 场景结果：按保守延展、相邻拓展、探索机会分成三组；
6. 场景卡片：说明、依据、推断、5–8 个商品角色；
7. 商品角色行：3–5 个可靠候选或新产品机会，并可展开子 SKU 变体和匹配理由。
