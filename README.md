...
本地前端为 React 工作台：选择国家 + 截图或商品名 → 一键生成经营建议与候选商品 → 按需调整与勾选 → 导出 Excel；没有识别后的人工确认关卡。流程见 [完整流程](docs/2026-09-22-flow-and-branches.md)。工作区与回退版本的恢复入口见 [UI 基准说明](docs/2026-09-22-ui-baseline.md)；已接入“首页管理店铺、完整分析与侧栏导航、场景默认全选、页尾导出”的实现说明见 [导出优先设计](docs/2026-09-22-export-first-design.md)。

```powershell
# 后端：FastAPI，监听 127.0.0.1:8000
# 在 UI 工作区执行，保留当前页面的数据目录；后端改码后重启此进程。
$env:SSI_DATA_DIR = Join-Path (Get-Location) 'var/demo'
.venv/Scripts/python.exe -m uvicorn store_scenario_inspiration.app.main:create_app --factory --host 127.0.0.1 --port 8000
